...
char temp_stack[MAX_STACK_SIZE];
int temp_stack_size;

int push_temp(char c)
{
   if(temp_stack_size  >=MAX_STACK_SIZE)
   {
        printf("Temp Push Increase MAX_STACK_SIZE size\n");
        return -2;
   }
   temp_stack[temp_stack_size] = c;
   temp_stack_size++;
   return 0;
}
...
char pop_temp()
{
  ...
  {
    printf("ERROR: Empty temp stack [size:%d]\n", temp_stack_size);
    return ret;
  }
  temp_stack_size--;
  ret = temp_stack[temp_stack_size];
  return ret;
}
...
int push(int crate, char c)
{
    if(crate >= MAX_CRATES)
    {
       printf("Push Increase MAX_CRATES size crate[%d]\n",crate);
        return -1;
    }

   if(crates_stack_size[crate] >= MAX_STACK_SIZE)
   {
        printf("Push Increase MAX_STACK_SIZE size[size:%d]\n",crates_stack_size[crate]);
        return -2;
   }
...
char pop(int crate)
{
  char ret = '\0';
    ...
    if(crate >= MAX_CRATES)
    {
       printf("Pop Increase MAX_CRATES size[crate:%d]\n", crate);
        return ret;
    }
